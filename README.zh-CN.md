...
- **AI 问答** — 在文档中直接向 AI 提问，回答以 Markdown 引用格式插入。支持"单独问题"和"全文上下文"两种模式
- **可配置快捷键** — 在设置面板中自定义 AI 问答快捷键（默认：`⌘E` / `⇧⌘E`）
...
1. 将光标放在包含问题的行上（或选中问题文字）
2. 按 **`⌘E`** 进行单独问答，或 **`⇧⌘E`** 进行全文上下文问答
3. 也可通过右键菜单：**「AI 问答」** / **「AI 问答（全文上下文）」**
4. AI 回答会以 Markdown 引用格式（`> ...`）插入到当前行下方
...
- **新增：AI 问答** — 在 Typora 文档中直接向 AI 提问，回答以 Markdown 引用格式插入到当前行下方
  - **单独问题模式** — AI 仅根据问题回答（默认快捷键：`⌘E`）
  - **全文上下文模式** — AI 以整篇文档为上下文回答（默认快捷键：`⇧⌘E`）
  - 同时可通过右键菜单使用
- **新增：可配置快捷键** — 在 AI 编辑设置面板中自定义问答快捷键，点击输入框后按下组合键即可录入
- **新增：问答提示词自定义** — 两种问答模式的 System / User Prompt 均可在设置面板中编辑
